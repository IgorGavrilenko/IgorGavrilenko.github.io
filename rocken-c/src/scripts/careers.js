window.addEventListener("DOMContentLoaded", () => {
    ...
    const workingEnvironment = document.querySelector(".working-environment");
    ...
    const yearsThumbs = new Swiper('.departments__slider__thumbs', {
        direction: "vertical",
        slidesPerView: 1,
        speed: 1000,
        simulateTouch: true,
    ...
    const yearsPictures = new Swiper('.departments__info', {
        slidesPerView: 1,
        effect: 'fade',
        speed: 1000,
        fadeEffect: {
    ...
    yearsThumbs.slides.forEach((slide, index) => {
    ...
    const totalSlides = yearsThumbs.slides.length;
    const triggerHeight = section.offsetHeight;
    console.log(section);
    ...
    ScrollTrigger.create({
        ...
        pin: true,
        scrub: 1,
        onLeave: () => { yearsThumbs.slideTo(totalSlides - 1); },
        onUpdate: (self) => {
            const slideIndex = Math.min(totalSlides - 1, Math.floor(self.progress * totalSlides));
